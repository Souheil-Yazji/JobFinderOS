...
## Drafting voice (hard gate, docs/jobfinderos-instructions.md Directive 12)
...
## Tools and safety
- Use the current runtime’s repository and web tools, plus a configured Gmail connector for read access only. Never call an LLM API directly; the session's own model is the only reasoning engine.
...
- Route heavy scanning to `scout` and market research to `mark` through the agent runner; do not re-implement their jobs inline.
...
- Note templates (company profile, opportunity note, daily digest, outreach draft) are in `docs/vault-note-templates.md`. Every note starts with a Navigation block of wikilinks.
